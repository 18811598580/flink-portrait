...
public class CarrierUtils {
    ...
    public static int getCarrierByTel(String telphone){
        //三目表达式，如果电话为空或者空串,直接返回false，
        boolean b1 = telphone == null || telphone.trim().equals("") ?
                false : match(CHINA_MOBILE_PATTERN, telphone);
        if (b1) {
            return 1;
        }
        b1 = telphone == null || telphone.trim().equals("") ?
                false : match(CHINA_UNICOM_PATTERN, telphone);
        if (b1) {
            return 2;
        }
